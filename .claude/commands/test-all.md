---
description: 백엔드 pytest(317) + 프론트엔드 vitest(168) 전체 테스트 실행 후 결과 보고 / Execute the full test suite (backend pytest 317 + frontend vitest 168) and report results
...
```bash
# Backend — pytest 317개 / 317 pytest tests
...
다음을 제시한다 / present:
- 스위트별 실행/통과/실패/스킵 수 (기대값: backend 317, frontend 168, 총 485)
  / per-suite totals: run, passed, failed, skipped (expected: backend 317, frontend 168, 485 total)
