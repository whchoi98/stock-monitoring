---
description: 백엔드 pytest(295) + 프론트엔드 vitest(110) 전체 테스트 실행 후 결과 보고 / Execute the full test suite (backend pytest 295 + frontend vitest 110) and report results
...
```bash
# Backend — pytest 295개 / 295 pytest tests
cd backend && .venv/bin/pytest -q
```

```bash
# Frontend — vitest 110개 (colocated .test.tsx) / 110 vitest tests
...
다음을 제시한다 / present:
- 스위트별 실행/통과/실패/스킵 수 (기대값: backend 295, frontend 110, 총 405)
  / per-suite totals: run, passed, failed, skipped (expected: backend 295, frontend 110, 405 total)
