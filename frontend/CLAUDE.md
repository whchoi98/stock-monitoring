...
## 구조 / Key Files
- `src/api/` — `client.ts`(fetch 래퍼), `queries.ts`(**모든 서버 데이터는 @tanstack/react-query 훅으로만**), `types.ts`(백엔드 `app/models.py` 응답 형태와 일치 유지).
...
npx vitest run         # 테스트 110개 (colocated *.test.tsx / *.test.ts)
npx oxlint             # 린트
npm run build:deploy   # vite build --outDir ../backend/static (emptyOutDir — 배포 산출물)
```

## 규칙 / Rules
- **TypeScript strict** + 함수형 컴포넌트만. 서버 상태는 react-query, 수동 fetch/useEffect 데이터 로딩 금지.
...
- 폰트는 Pretendard (`@fontsource/pretendard`). AI 응답 렌더링은 react-markdown.
