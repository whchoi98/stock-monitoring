...
- 테스트: 백엔드 364 → 380 (한글명 커버리지, 질문 정규화·키·캐시·422·제어문자·울타리 전각화, 프롬프트 격리/절단, `name_ko` 응답), 프론트 280 → 301 (한글·초성·혼합 매칭, AIPanel 질문·프리셋·잠금·재시도, MACD 임계값).
- 배포 전 적대적 리뷰(워크플로 52 에이전트, 4차원 × 검증자 3명): 확정 4건 수정 — 공백 포함 한글명 초성 검색(`matchHangul`로 일반화, IME 조합 중 혼합도 지원), `</question>` 울타리 닫기(`<`/`>` 전각화, 모델·프롬프트 2중), MACD 최소 캔들 35→34, 라틴 폴백 한글명 중복 표기(한글 음절 있는 이름만 `name_ko`로 내보냄).
