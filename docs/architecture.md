...
- **SSRF guards** -- `news.fetch_article_content` takes client URLs: scheme allow-list (http/https), private/loopback/link-local IP rejection, 2 MB streaming cap on decompressed bytes (with 16 MB raw-read and 64 KB per-step decompression bounds, a 20 s total fetch deadline and a charset whitelist), and backtracking-bounded regex (`[^<>]{0,1000}`) to prevent O(n²) DoS.
...
- **SSRF/DoS guards on article fetch** -- the URL is client-supplied: scheme allow-list, private-IP rejection, 2 MB streaming cap (bounded decompression, 20 s total deadline, charset whitelist), and backtracking-bounded regex block SSRF and O(n²) regex DoS.
...
- **SSRF 가드** -- `news.fetch_article_content`는 클라이언트 URL을 받는다: 스킴 허용 목록(http/https), private/loopback/link-local IP 거부, 2MB 스트리밍 캡(해제 바이트 기준; 원시 읽기 16MB·스텝 64KB 해제 상한, fetch 총 데드라인 20초, charset 화이트리스트 포함), 백트래킹 상한 regex(`[^<>]{0,1000}`)로 SSRF와 O(n²) DoS를 차단한다.
...
- **기사 조회 SSRF/DoS 가드** -- URL이 클라이언트에서 온다: 스킴 허용 목록, private IP 거부, 2MB 스트리밍 캡(압축 해제 상한·총 데드라인 20초·charset 화이트리스트 포함), 백트래킹 상한 regex로 SSRF와 O(n²) regex DoS를 차단한다.
