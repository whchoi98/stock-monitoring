...
- **backend/app/services/bedrock_ai.py** -- Bedrock `converse_stream` calls (`stream_invoke`, the sole call primitive: a pump thread feeds an asyncio queue) for stock analysis and article summary/translation, with typed errors (`BedrockUnavailableError` / `BedrockCallError`).
...
- **backend/app/api/ai.py** -- `/api/ai/stocks/{symbol}` and `/api/ai/articles` with the three-way cost defense (rate limit → result cache → global concurrency cap). Both routes answer `text/event-stream` (`phase` → `delta`* → `final`, a `final` on every path; only 429/422/404 stay plain JSON before the stream). `POST /api/ai/stocks/{symbol}` takes an optional `{"question"}` body (≤ 200 chars, normalised and fenced in the prompt) cached under `ai:stock:{symbol}:q:{sha256[:16]}`.
...
- **SSRF guards** -- `news.fetch_article_content` takes client URLs: scheme allow-list (http/https), private/loopback/link-local IP rejection, 2 MB streaming cap on decompressed bytes (with 16 MB raw-read and 64 KB per-step decompression bounds, a 20 s total fetch deadline and a charset whitelist), and tag regexes that exclude `<` (linear without a length cap) to prevent O(n²) DoS.
...
flowchart TB
  ...
  subgraph security["Security Layer"]
    ...
    ssrf["SSRF guard + 2MB cap<br/>fetch_article_content"]
  end
...
| Task role policy | — | DynamoDB `GetItem/PutItem/BatchGetItem/BatchWriteItem/Query` scoped to the table ARN; `bedrock:InvokeModel` + `bedrock:InvokeModelWithResponseStream` on `*` (the `global.` profile resolves to cross-region model ARNs; `converse_stream` is authorised as the latter) |
...
| CloudFront distribution | `d2wa9w1vbqlndl` | Default behavior: `CACHING_DISABLED`, `ALLOW_ALL` methods, compress, redirect-to-https, HTTP-only origin (ALB has no cert) with `read_timeout` 60 s; `/assets/*`: `CACHING_OPTIMIZED` |
...
- **SSRF/DoS guards on article fetch** -- the URL is client-supplied: scheme allow-list, private-IP rejection, 2 MB streaming cap (bounded decompression, 20 s total deadline, charset whitelist), and `<`-excluding tag regexes (linear without a length cap) block SSRF and O(n²) regex DoS.
...
- **backend/app/services/bedrock_ai.py** -- 종목 분석·기사 요약/번역용 Bedrock `converse_stream` 호출(`stream_invoke` — 유일한 호출 프리미티브, 전용 스레드가 asyncio 큐로 전달). 타입 있는 예외(`BedrockUnavailableError` / `BedrockCallError`)를 낸다.
...
- **backend/app/api/ai.py** -- `/api/ai/stocks/{symbol}`, `/api/ai/articles`. 3중 비용 방어(레이트리밋 → 결과 캐시 → 전역 동시 실행 제한). 두 라우트는 `text/event-stream`(`phase` → `delta`* → `final`, 어느 경로든 `final` 보장; 429/422/404만 스트림 전 JSON)으로 응답하고, 종목 라우트는 선택 본문 `{"question"}`(200자 이하, 정규화·프롬프트 울타리)을 받아 `ai:stock:{symbol}:q:{sha256[:16]}` 키로 캐시한다.
...
- **SSRF 가드** -- `news.fetch_article_content`는 클라이언트 URL을 받는다: 스킴 허용 목록(http/https), private/loopback/link-local IP 거부, 2MB 스트리밍 캡(해제 바이트 기준; 원시 읽기 16MB·스텝 64KB 해제 상한, fetch 총 데드라인 20초, charset 화이트리스트 포함), `<`를 제외한 태그 regex(길이 상한 없이 선형)로 SSRF와 O(n²) DoS를 차단한다.
...
| 태스크 롤 정책 | — | DynamoDB `GetItem/PutItem/BatchGetItem/BatchWriteItem/Query` 테이블 ARN 한정; `bedrock:InvokeModel` + `bedrock:InvokeModelWithResponseStream`은 `*`(`global.` 프로파일이 교차 리전 모델 ARN으로 해석되므로; `converse_stream`은 후자로 평가) |
...
| CloudFront 배포 | `d2wa9w1vbqlndl` | 기본 동작: `CACHING_DISABLED`, `ALLOW_ALL` 메서드, 압축, redirect-to-https, HTTP-only 오리진(ALB에 인증서 없음), `read_timeout` 60초; `/assets/*`: `CACHING_OPTIMIZED` |
...
- **가격 오버레이** -- 장중 45초마다 갱신되는 quotes 캐시가 응답 시점에 12시간 detail 캐시의 `price/change/change_pct/volume`을 덮어쓴다. 대시보드 테이블, 상세 헤더, 호가가 항상 같은 가격을 보여주면서 펀더멘털 재조회는 하지 않는다.
...
- **기사 조회 SSRF/DoS 가드** -- URL이 클라이언트에서 온다: 스킴 허용 목록, private IP 거부, 2MB 스트리밍 캡(압축 해제 상한·총 데드라인 20초·charset 화이트리스트 포함), `<`를 제외한 태그 regex(길이 상한 없이 선형)로 SSRF와 O(n²) regex DoS를 차단한다.
