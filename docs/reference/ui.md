...
| Price chart | `frontend/src/components/stock/PriceChart.tsx` | lightweight-charts candles + MA5/MA20 + Bollinger + golden/dead-cross markers + direction-tinted volume; period tabs 1W/1M/3M/6M/1Y/5Y (5Y weekly); toggles MA5/MA20/BOLL/VOL/LVL/RSI/MACD — LVL draws dotted reference lines (previous close, 52-week high/low), RSI 14 and MACD 12·26·9 are separate `createChart` sub-panes whose time scale and crosshair are synced with the main chart; an OHLC legend follows the hovered candle and a 캔들/표 switch swaps the canvas for `CandleTable`. Reads `--up/--down/--chart-*/--panel/--border/--border-strong/--text-dim/--text-strong/--font-mono` via `getComputedStyle` (the one sanctioned colour exception) and rebuilds on a theme change |
...
| 가격 차트 | `frontend/src/components/stock/PriceChart.tsx` | lightweight-charts 캔들 + MA5/MA20 + 볼린저 + 골든/데드 크로스 마커 + 방향색 거래량. 기간 탭 1W/1M/3M/6M/1Y/5Y(5Y는 주봉), 토글 MA5/MA20/BOLL/VOL/LVL/RSI/MACD — LVL은 점선 기준선(전일 종가·52주 고/저), RSI 14·MACD 12·26·9는 시간축·크로스헤어가 메인과 동기화된 별도 `createChart` 보조 패널. OHLC 레전드가 호버 캔들을 따르고 캔들/표 토글은 캔버스를 `CandleTable`로 바꾼다. `--up/--down/--chart-*/--panel/--border/--border-strong/--text-dim/--text-strong/--font-mono`를 `getComputedStyle`로 읽는다(승인된 유일한 색 예외), 테마가 바뀌면 재생성 |
