...
| Price chart | `frontend/src/components/stock/PriceChart.tsx` | lightweight-charts candles + MA5/MA20 + Bollinger + direction-tinted volume; reads `--up/--down/--chart-*/--panel/--border/--text-dim/--font-mono` via `getComputedStyle` (the one sanctioned colour exception) and rebuilds on a theme change |
...
| 가격 차트 | `frontend/src/components/stock/PriceChart.tsx` | lightweight-charts 캔들 + MA5/MA20 + 볼린저 + 방향색 거래량. `--up/--down/--chart-*/--panel/--border/--text-dim/--font-mono`를 `getComputedStyle`로 읽는다(승인된 유일한 색 예외), 테마가 바뀌면 재생성 |
