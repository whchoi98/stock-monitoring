...
| Input caps | `backend/app/api/ai.py` | `MAX_URL_LEN` 2048 / `MAX_TITLE_LEN` 512 — client input reaches prompts and cache keys, so its length is bounded |
...
| 입력 상한 | `backend/app/api/ai.py` | `MAX_URL_LEN` 2048 / `MAX_TITLE_LEN` 512 — 클라이언트 입력이 프롬프트·캐시 키에 들어가므로 길이 제한 |
