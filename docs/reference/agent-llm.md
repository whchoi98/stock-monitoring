...
| AI routes | `backend/app/api/ai.py` | `POST /api/ai/stocks/{symbol}` (optional body `{question}`, 1–200 chars) and `POST /api/ai/articles` (`{url, title, language}`); enforces the defense order and the fixed error bodies |
...
- **Defense order is fixed**: (1) rate limit — *before* the cache, so even cache hits spend budget and one IP cannot poll without bound; (2) result cache (`AI_TTL` 6h); (3) Bedrock inside the global semaphore (2 concurrent), the permit held until the stream ends; the article body is fetched beforehand under its own `AI_FETCH_CONCURRENCY` permit, released as soon as the body is in. Waiting for either permit heartbeats `phase: waiting`. The route's in-flight registry additionally collapses concurrent identical requests into one Bedrock stream: one leader streams and caches, the others heartbeat `phase: waiting` and inherit its outcome.
...
| AI 라우트 | `backend/app/api/ai.py` | `POST /api/ai/stocks/{symbol}`(선택 본문 `{question}`, 1~200자), `POST /api/ai/articles`(`{url, title, language}`). 방어 순서와 고정 오류 본문 강제 |
...
- **방어 순서는 고정**: ① 레이트리밋 — 캐시보다 **앞**이라 캐시 히트도 예산을 소비, 한 IP가 무한 폴링 불가. ② 결과 캐시(`AI_TTL` 6h). ③ 전역 세마포어(동시 2) 안에서 Bedrock 호출, permit은 스트림 완료까지 보유. 기사 본문은 그 전에 별도 `AI_FETCH_CONCURRENCY` permit 안에서 조회하고 본문이 들어오면 즉시 반납한다. 두 permit 대기 모두 `phase: waiting` 하트비트를 낸다. 라우트의 진행 중 스트림 레지스트리가 추가로 동일 키 동시 요청을 Bedrock 1회로 합친다 — 선점자가 스트리밍·캐싱하고 나머지는 `phase: waiting` 하트비트 후 그 결과를 승계한다.
- **캐시 키**: `ai:stock:{symbol}`, `ai:stock:{symbol}:q:{sha256(question)[:16]}`, `ai:article:{sha1(url)[:16]}` — 기사 키는 URL만 사용 (같은 URL + 다른 제목이면 먼저 생성된 분석 반환; 원본 URL은 키에 들어가지 않음). 질문 키는 **정규화된** 질문을 해시하므로 같은 질문은 6시간 캐시를 공유하고 기본 분석 캐시는 건드리지 않는다.
- **자유 질의 (2026-09-07)**: `{question}`이 있으면 `_stock_prompt`가 답변/근거/리스크 형식으로 바뀐다. 질문은 사용자 입력이므로 ① pydantic 검증기가 정규화(제어문자 제거·꺾쇠 `<`/`>` 전각화로 울타리 닫기 차단·공백 접기·빈 값 거절·`MAX_QUESTION_LEN` 200), ② 프롬프트에서 한 번 더 자르고(`QUESTION_LIMIT`), ③ `<question>` 구분자 안에 격리해 내부 지시 무시·데이터 범위 유지·매매 지시 금지를 명시한다. 레이트리밋·동시성 상한·TTL은 기본 분석과 같다 — 비용 변화는 "질문마다 캐시 항목이 다르다"는 것 하나이고 IP별 한도가 그 폭을 묶는다.
