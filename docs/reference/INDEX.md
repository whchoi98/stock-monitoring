...
| API | [api.md](api.md) | FastAPI 라우트 전체, envelope 규약, 심볼 유니버스 검증, 고정 오류 문구, SPA 서빙 |
| IaC | [iac.md](iac.md) | CDK v2 단일 스택, VPC lookup pin, 오리진 시크릿, origin request policy, 최소 권한 IAM |
| Frontend | [frontend.md](frontend.md) | React 19 SPA 구조, TanStack Query 훅·폴링 상수, ApiError 분기, 빌드/배포 경로 |
| UI | [ui.md](ui.md) | 디자인 토큰(tokens.css), 다크/라이트 테마, 상승=빨강/하락=파랑 한국 관례, Pretendard |
| Security | [security.md](security.md) | 오리진 검증, AI 레이트리밋 키(CloudFront-Viewer-Address), SSRF 가드, regex DoS 상한, defusedxml |
