...
| Components | `frontend/src/components/` | `common/` (Panel — collapsible by `id`, Stat, MarketStrip, SymbolSearch, StatusBar, MarketStatus, Clock, ScopeTabs, StarButton, NewsList, AlertsWatcher — toasts, UpdateToast — service-worker toasts (new version / offline ready), ChangeText, ThemeToggle, badges, ErrorCard, Spinner), `market/` (MarketPulse, SectorBars, MacroPanel, StockTable, NewsFeed), `stock/` (Watchlist, StockHeader, AlertForm, PriceChart, CandleTable, OrderBook, InvestorPanel, FundamentalCards, ReturnsRow, StockNews, AIPanel, Week52Bar) |
| Utilities | `frontend/src/lib/` | `format.ts` (number/price formatting), `clock.ts` (HH:MM / KST HH:MM:SS), `search.ts` (`searchSymbols` ranking: exact symbol > symbol prefix > name prefix — Latin, or Korean `name_ko` via `hangul.ts`'s `matchHangul`: syllables, initials (ㅅㅅㅈㅈ) and the mixed forms an IME emits mid-composition (삼ㅅ), spaces ignored on both sides — > substring; KR codes match without `.KS/.KQ`), `markets.ts` (market labels, `QuoteScope` = market or `watch`), `scopedQuotes.ts` (`useScopedQuotes`: one market's quotes or the starred symbols from both), `newsFilter.ts` (`filterNews`: language tab + title keyword), `localStore.ts` + `watchlistStore.ts` / `alertsStore.ts` / `panelStore.ts` (browser-only user state on `useSyncExternalStore`; `evaluateAlerts` is pure), `online.ts` (`useOnline`: `navigator.onLine` on `useSyncExternalStore`, the status bar's offline badge), `stickyOffsets.ts` (`observeStickyOffsets`: the sticky top block's and status bar's measured heights as `--sticky-top-h` / `--sticky-bottom-h`, the toast anchors — the tokens are desktop values and both blocks wrap on narrow screens), `aiMessages.ts` (AI error → user wording + `phase` labels), `articleLink.ts` (news-link fork: analysis screen vs. source in a new tab), `sse.ts` (incremental SSE frame parser) |
...
| Build config | `frontend/vite.config.ts` | Dev proxy `/api → http://localhost:8000`; vitest jsdom + `globals: true` (testing-library auto-cleanup needs a global `afterEach`); `VitePWA` (ADR-002): manifest + workbox `generateSW`, prompt-mode updates, `/api/` denylisted from the navigate fallback, fonts runtime-cached |
...
- `frontend/vite.config.ts` — proxy + vitest `globals` rationale; the `VitePWA` block and its comment (worker scope = app shell only)
...
- Related ADRs: [ADR-001](../decisions/ADR-001-terminal-design-language.md) (terminal design language), [ADR-002](../decisions/ADR-002-pwa-app-shell.md) (PWA app-shell worker) — design spec `docs/superpowers/specs/2026-08-01-stock-monitoring-design.md`
...
| 컴포넌트 | `frontend/src/components/` | `common/`(Panel — `id`로 접기, Stat, MarketStrip, SymbolSearch, StatusBar, MarketStatus, Clock, ScopeTabs, StarButton, NewsList, AlertsWatcher — 토스트, UpdateToast — 서비스 워커 토스트(새 버전 / 오프라인 준비), ChangeText, ThemeToggle, 배지, ErrorCard, Spinner), `market/`(MarketPulse, SectorBars, MacroPanel, StockTable, NewsFeed), `stock/`(Watchlist, StockHeader, AlertForm, PriceChart, CandleTable, OrderBook, InvestorPanel, FundamentalCards, ReturnsRow, StockNews, AIPanel, Week52Bar) |
| 유틸리티 | `frontend/src/lib/` | `format.ts`(숫자/가격 포맷), `clock.ts`(HH:MM / KST HH:MM:SS), `search.ts`(`searchSymbols` 순위: 심볼 정확 > 심볼 접두 > 종목명 접두 — 영문, 또는 `hangul.ts`의 `matchHangul`로 맞춘 한글 `name_ko`(음절·초성 ㅅㅅㅈㅈ·IME 조합 중 혼합 삼ㅅ, 공백은 양쪽 무시) — > 포함. KR 코드는 `.KS/.KQ` 없이도 매칭), `markets.ts`(시장 라벨, `QuoteScope` = 시장 또는 `watch`), `scopedQuotes.ts`(`useScopedQuotes`: 한 시장 시세 또는 두 시장의 ★ 종목), `newsFilter.ts`(`filterNews`: 언어 탭 + 제목 키워드), `localStore.ts` + `watchlistStore.ts` / `alertsStore.ts` / `panelStore.ts`(`useSyncExternalStore` 위의 브라우저 전용 사용자 상태. `evaluateAlerts`는 순수), `online.ts`(`useOnline`: `useSyncExternalStore` 위의 `navigator.onLine`, 상태 바 오프라인 배지), `stickyOffsets.ts`(`observeStickyOffsets`: 상단 고정 블록·상태 바의 실측 높이를 `--sticky-top-h` / `--sticky-bottom-h`로 — 토스트 앵커. 토큰은 데스크톱 값이고 두 블록은 좁은 화면에서 줄바꿈한다), `aiMessages.ts`(AI 오류 → 사용자 문구 + `phase` 라벨), `articleLink.ts`(뉴스 링크 분기 — 분석 화면 vs 원문 새 탭), `sse.ts`(SSE 프레임 파서) |
...
| 빌드 설정 | `frontend/vite.config.ts` | dev 프록시 `/api → http://localhost:8000`. vitest jsdom + `globals: true` (testing-library 자동 cleanup은 전역 `afterEach` 필요). `VitePWA`(ADR-002): 매니페스트 + workbox `generateSW`, prompt 방식 업데이트, navigate fallback에서 `/api/` 제외, 폰트 런타임 캐시 |
...
- `frontend/vite.config.ts` — 프록시 + vitest `globals` 근거. `VitePWA` 블록과 주석(워커 범위 = 앱 셸만)

### 5. 상호 참조
- 관련 모듈: [api.md](api.md) (소비하는 envelope 계약), [ui.md](ui.md) (컴포넌트가 써야 하는 토큰/테마), [agent-llm.md](agent-llm.md) (AI 패널 동작)
- 관련 ADR: [ADR-001](../decisions/ADR-001-terminal-design-language.md)(터미널 디자인 언어), [ADR-002](../decisions/ADR-002-pwa-app-shell.md)(PWA 앱 셸 워커) — 설계 스펙 `docs/superpowers/specs/2026-08-01-stock-monitoring-design.md`
- 관련 런북: 아직 없음
