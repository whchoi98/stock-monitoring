...
React Router **7.18.3** is deployed to the existing `StockMonitoringStack` (`ap-northeast-2`, task revision 13). Verification passed **437 backend + 452 frontend + 15 browser tests = 904**, strict type checks, build and lint; npm audit reported **0 findings**. See the [documentation index](docs/README.md), [quality changes](docs/quality-upgrade-2026-09-13.md) and [deployment evidence](docs/deployments/2026-09-13-router7-quality-upgrade.md).

## Features

- **Market workbench** — prominent US/KR/watch navigation preserved in the URL, a tracked-universe breadth summary including unchanged stocks, and clickable leaders. Combine unlimited symbol/Korean-name/initial search with sector and movement filters, sort missing values last, export the displayed rows as UTF-8 CSV, and remember table density. Phones offer a compact view plus “전체 열” for all columns and sorting.
- **Recoverable monitoring** — failed refreshes retain the last data alongside a retry notice; both watchlist markets refresh every 45 seconds, partial results stay usable, and in-flight lookups are distinguished from empty results. GET requests have a 30-second deadline and cancellation. The strip, news timestamps and footer clock use KST.
...
- **Article-analysis entry** — Open `/articles`, enter a public article URL, optionally add its title and select the language, then submit. Opening the form makes no AI request; article links reuse the same SSE result screen.
...
stock-monitoring/
  ...
  frontend/              # React 19 + TypeScript + Vite 8
    src/
      ...
      pages/             # Dashboard, StockDetail, ArticleAnalysis + ArticleStart form
      styles/            # tokens.css, global.css, workspace.css
    e2e/                 # 15 Chromium scenarios; snapshot API fixtures, no Yahoo/AWS calls
  ...
  docs/                  # architecture.md, api-reference.md, reference/ (per-layer), decisions/ (ADR-001 terminal UI, ADR-002 PWA, ADR-003 workbench), deployments/, runbooks/, superpowers/ (specs + plans)
  tests/                 # Harness shell tests (hooks, project structure)
  .github/workflows/     # CI: backend pytest + frontend tsc / oxlint / vitest / Playwright
...
```bash
# Run all tests (backend + frontend) — 437 pytest + 452 vitest
make test

# Backend only (pytest, 437 tests)
cd backend && .venv/bin/pytest -q

# Frontend only (vitest, 452 tests) + type check + lint
cd frontend && npx vitest run && npx tsc -b && npm run lint

# CI also builds and runs the 15 browser scenarios, retaining reports for 7 days
```

Browser regression checks build the production app and start their own server on port 4317. They use public market snapshots and synthetic AI responses, without calling Yahoo/AWS:
...
See [quality upgrade notes](docs/quality-upgrade-2026-09-13.md) for changes, evidence and the Router 7 migration and deployment results.
...
React Router **7.18.3**을 기존 `StockMonitoringStack`(`ap-northeast-2`, 태스크 리비전 13)에 배포했습니다. **백엔드 437 + 프런트엔드 452 + 브라우저 15 = 904개 테스트**, 타입 검사·빌드·린트를 통과했고 npm audit 결과는 **0건**입니다. [문서 색인](docs/README.md), [품질 개선 내용](docs/quality-upgrade-2026-09-13.md), [운영 배포 검증](docs/deployments/2026-09-13-router7-quality-upgrade.md)을 참고하세요.

## 주요 기능

- **시장 탐색 도구** — 상단의 미국/한국/관심 선택을 URL에 보존하고, 보합을 포함한 추적 종목 요약과 상세로 연결되는 순위 목록을 제공합니다. 한글·초성·영문 검색, 섹터·등락 필터, 결측값을 마지막에 두는 정렬, 화면 순서 그대로의 CSV 내보내기, 표 밀도 저장을 지원합니다. 모바일의 “전체 열” 버튼으로 모든 컬럼과 정렬에 접근할 수 있습니다.
- **갱신 실패에서 복구** — 마지막 데이터를 유지하면서 재시도를 제공하고, 관심 목록의 두 시장을 모두 45초마다 갱신합니다. 일부 시장만 도착한 상태, 조회 실패, 빈 결과를 구분하며 GET 요청은 30초 제한시간과 취소를 지원합니다. 스트립·뉴스·상태 바 시각은 KST로 통일했습니다.
...
- **기사 분석 입력 화면** — `/articles`에서 공개 기사 URL·선택 제목·언어를 입력하고 제출합니다. 입력 화면을 열기만 해서는 AI를 호출하지 않으며, 뉴스 링크도 같은 SSE 결과 화면을 사용합니다.
...
- **관심 종목 ★·가격 알림 (브라우저 전용)** — 시세 표·워치리스트 레일·종목 헤더의 ★로 관심 종목을 모으고, 상향/하향 목표가 알림을 걸면 어느 화면에서든 양 시장을 폴링하며 판정해 한 번만 울린다(권한 허용 시 시스템 알림, 아니면 토스트). 관심 종목·알림·패널 접힘 상태는 `localStorage`에만 저장되며 백엔드는 모른다
...
- **설치형 PWA (ADR-002)** — 웹 앱 매니페스트(다크 테마 색, 브랜드 마크 아이콘·maskable 포함)와 앱 셸만 담당하는 workbox 서비스 워커: 빌드 산출물 프리캐시, SPA 경로 오프라인 서빙, 폰트는 첫 사용 시 캐시, `/api/*`는 절대 캐시하지 않음(오프라인에서는 폴링이 멈춰 마지막 데이터 위에 상태 바 배지, 오프라인 최초 조회는 연결 복구 후 진행); 새 버전은 업데이트 토스트의 "새로 고침"을 눌러야 적용
...
stock-monitoring/
  ...
  docs/                  # architecture.md, api-reference.md, reference/(계층별), decisions/(ADR-001 터미널 UI, ADR-002 PWA, ADR-003 workbench), deployments/, runbooks/, superpowers/(스펙+계획)
  tests/                 # 하네스 셸 테스트 (훅, 프로젝트 구조)
  .github/workflows/     # CI: 백엔드 pytest + 프론트 tsc / oxlint / vitest / Playwright
...
브라우저 회귀 테스트는 프로덕션 앱을 빌드하고 전용 4317 포트에서 실행합니다. 시장 스냅샷과 테스트용 AI 응답을 사용하므로 Yahoo/AWS를 호출하지 않습니다.

```bash
cd frontend
npx playwright install --with-deps chromium  # 환경별 최초 1회
npm run test:e2e
```

변경 내용·검증 결과·Router 7 보안 업데이트와 운영 배포는 [품질 개선 기록](docs/quality-upgrade-2026-09-13.md)을 참고하세요.
